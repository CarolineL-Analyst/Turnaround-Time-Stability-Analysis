...
While TRT exists across domains (e.g., ticket resolution), this analysis is optimized for **production and MRO environments**, where variability directly impacts asset availability and delivery commitments.

## Overview

This project analyzes Turnaround Time (TRT) behavior at the **Part Number + Workscope** level to assess **process stability, volatility, and anomaly patterns over time**.
...
- Has TRT remained broadly stable over time for the same Part Number and Workscope?
...
- Metrics are aggregated at the **Part Number + Workscope + reporting period** level.
...
- Each record represents a completed job with Part Number, Workscope, start date, and completion date
...
This analysis assumes that, under normal operating conditions, TRT for the same Part Number and Workscope should exhibit relative stability.
